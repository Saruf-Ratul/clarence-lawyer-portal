...
public record CaseDetailDto(Guid Id, string CaseNumber, string ClientName, string PropertyAddress, string TenantName, string Unit, decimal MonthlyRent, decimal BalanceOwed, decimal LateFees, decimal CourtCosts, DateOnly? NextDeadline, string Stage, IReadOnlyList<TimelineEventDto> Timeline);
...
public record RmImportRequest(string RequestedBy);
public record RmImportResult(int ImportedSubmissions, int ImportedClients, int ImportedProperties, int ImportedTenants);

public record NjFormDto(Guid CaseId, string CaseNumber, string PlaintiffName, string TenantName, string PropertyAddress, string Unit, decimal MonthlyRent, decimal BalanceOwed, decimal LateFees, decimal CourtCosts, DateOnly? NextCourtDate, string County, DateTime GeneratedAtUtc);
...
public interface IRentManagerSyncService { Task<RmImportResult> PullLatestAsync(RmImportRequest request, CancellationToken cancellationToken = default); }
public interface IFormService { Task<NjFormDto?> BuildNjLtFormAsync(Guid caseId, CancellationToken cancellationToken = default); }
